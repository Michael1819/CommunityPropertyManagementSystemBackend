...
CREATE
OR REPLACE FUNCTION insert_bill()
RETURNS TRIGGER AS $$
BEGIN
    IF
TG_OP = 'INSERT' THEN
...
$$
LANGUAGE plpgsql;
CREATE TRIGGER insert_bill_trigger
    AFTER INSERT
    ON bills
    FOR EACH ROW EXECUTE FUNCTION insert_bill();
CREATE
OR REPLACE FUNCTION insert_payment()
RETURNS TRIGGER AS $$
BEGIN
    IF
TG_OP = 'INSERT' THEN
...
CREATE TRIGGER insert_payment_trigger
    AFTER INSERT
    ON payments
    FOR EACH ROW EXECUTE FUNCTION insert_payment();
...
INSERT INTO facilities (facility_name, description) VALUES ('324', '234')
