...
func (backend *PostgresBackend) InsertReservation(username string, facilityName string, remark string, reservationDate string, startHour int, endHour int) (*model.Reservation, error) {
	...
	//	return nil, err
	//}
	query := "INSERT INTO facilities (facility_name, description) VALUES ($1, $2) RETURNING id"

	var facilityId int64
	err := tx.QueryRow(query, facilityName, facilityName).Scan(&facilityId)
	if err != nil {
		log.Println(err)
		return nil, err
	}
